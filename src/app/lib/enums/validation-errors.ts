...
export const ValidationErrorsEnum: any = {
  ...
  minlength({ requiredLength, actualLength }: { requiredLength: number; actualLength: number }) {
    return { ar: `يجب تعبئة هذا الحقل بحد اقل ${requiredLength}`, en: `You must fill this field with a minimum of ${requiredLength}` };
  },
  maxlength({ requiredLength, actualLength }: { requiredLength: number; actualLength: number }) {
    return { ar: `يجب تعبئة هذا الحقل بحد اقصي ${requiredLength}`, en: `You must fill this field with a maximum of ${requiredLength}` };
  },
  ...
  min({ min, actual }: { min: number; actual: string }) {
    return { ar: `القيمة المدخلة اقل من الحد الادنى (${min})`, en: `The entered value is less than the minimum value (${min})` };
  },
  max({ max, actual }: { max: number; actual: string }) {
    return { ar: `القيمة المدخلة أكبر من الحد الأقصى (${max})`, en: `The entered value is greater than the maximum value (${max})` };
  },
  email() {
