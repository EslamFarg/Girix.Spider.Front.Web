...
@Component({
  selector: 'app-explorer-incomming-transfer',
  imports: [PageHeaderSearch, Paginator, SharedConfirmDialog, NgClass],
  templateUrl: './explorer-incomming-transfer.html',
  styleUrl: './explorer-incomming-transfer.scss',
})
export class ExplorerIncommingTransfer implements OnInit {
  ...
  loadData(): void {
    ...
    const request$ = this.activeSearchCode
      ...
      : this._transferRequestService.getAllListTransferRequest(
          TransferRequestViewType.IncomingReview,
          page,
          this.rows,
        );
  ...
  onEditData(id: number): void {
    ...
    this._router.navigate(['/incomming-transfer/add']);
  }
