...
export class JournalForm extends BaseComponent {
    ...
    financialAccounts = computed(() =>
        this._financialAccounts().map((a) => ({
            ...a,
            label: `${a.name} - ${a.finNumber}`,
        })),
    );
    ...
    createNewJournalDetailsItemFg(data?: IAppJournalItem) {
        const fg = this.fb.group<IAppJournalItemControls>({
            creditorAmount: this.fb.control<number>(data?.creditorAmount ?? 0, [
                Validators.required,
                onlyNumbersOrDotAllowed,
            ]),
            debtorAmount: this.fb.control<number>(data?.debtorAmount ?? 0, [
                Validators.required,
                onlyNumbersOrDotAllowed,
            ]),
