...
export class ProductForm extends BaseComponent implements OnInit {
        ...
        if (isImageNew) this.newImages.update((images) => images.filter((i) => i.id !== this.currentImage()?.id));
            ...
            this.productFg.patchValue({
                listIdsOfDeleteImages: [
                    ...this.productFg.value.listIdsOfDeleteImages!,
                    this.currentImage()?.id as number,
                ],
            });
        }
        this.currentImage.set(this.allImages()[0] ?? null);
    ...
    getNextGroupsPage(event: SelectLazyLoadEvent) {
        ...
        if (event.last === this.groups().length)
            this.emitDebounceItem('searchGroups', {
                pageIndex: this.groupsPaginationInfo.pageIndex + 1,
                searchValue: '',
            });
    }
    ...
    deleteProduct(id: number, event: Event) {
        this.confirmationService.confirm({
            ...
            accept: () => {
                this.productService
                    .delete(id)
                    .subscribe({ next: () => this.router.navigate(['/classes/products/add']) });
            },
        });
    }
}
