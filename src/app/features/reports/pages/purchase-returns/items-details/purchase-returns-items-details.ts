...
import { IPurchaseReturnItemDetailRow } from '../../../types/api/reports-types';

@Component({
  selector: 'app-purchase-returns-items-details-report',
  ...
  templateUrl: './purchase-returns-items-details.html',
  styleUrl: './purchase-returns-items-details.css',
})
export class PurchaseReturnsItemsDetails extends BaseComponent {
  ...
  columns: IReportColumn[] = [
    ...
    { key: 'lineTotal', label: 'الاجمالي', type: 'currency' },
  ];

  rows = signal<IPurchaseReturnItemDetailRow[]>([]);
  ...
  search(pageIndex: number) {
    ...
    this.reportsService.getPurchaseReturnsItemsDetails({ ...v, pageIndex, pageSize: 10 }).subscribe({
      next: (res) => {
