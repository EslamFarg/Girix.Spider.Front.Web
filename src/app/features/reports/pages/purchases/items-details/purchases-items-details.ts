...
import { IPurchaseItemDetailRow } from '../../../types/api/reports-types';

@Component({
  selector: 'app-purchases-items-details-report',
  ...
  templateUrl: './purchases-items-details.html',
  styleUrl: './purchases-items-details.css',
})
export class PurchasesItemsDetails extends BaseComponent {
  ...
  columns: IReportColumn[] = [
    ...
    { key: 'lineTotal', label: 'القيمة', type: 'currency', total: true },
  ];

  rows = signal<IPurchaseItemDetailRow[]>([]);
  ...
  search(pageIndex: number) {
    ...
    this.reportsService.getPurchasesItemsDetails({ ...v, pageIndex, pageSize: 10 }).subscribe({
      next: (res) => {
